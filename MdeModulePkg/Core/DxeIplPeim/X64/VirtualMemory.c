  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "DxeIpl(%d): 0x%x bytes needed for page table\n", __LINE__, PageTableBufferSize));
  ...
  if (IsNullDetectionEnabled ()) {
    PageTableBufferSize = 0;
    Status = PageTableMap (&PageTable, PagingMode, NULL, &PageTableBufferSize, 0, SIZE_4KB, &MapAttribute, &MapMask);
    ASSERT (Status == RETURN_BUFFER_TOO_SMALL);
    DEBUG ((DEBUG_INFO, "DxeIpl(%d): 0x%x bytes needed for page table\n", __LINE__, PageTableBufferSize));
    PageTableBuffer = AllocatePageTableMemory (EFI_SIZE_TO_PAGES (PageTableBufferSize));
    ASSERT (PageTableBuffer != NULL);
    Status = PageTableMap (&PageTable, PagingMode, PageTableBuffer, &PageTableBufferSize, 0, SIZE_4KB, &MapAttribute, &MapMask);
    ASSERT (Status == RETURN_SUCCESS);
    ASSERT (PageTableBufferSize == 0);
  }
  if (PcdGetBool (PcdCpuStackGuard)) {
    PageTableBufferSize = 0;
    Status = PageTableMap (&PageTable, PagingMode, NULL, &PageTableBufferSize, StackBase, SIZE_4KB, &MapAttribute, &MapMask);
    ASSERT (Status == RETURN_BUFFER_TOO_SMALL);
    DEBUG ((DEBUG_INFO, "DxeIpl(%d): 0x%x bytes needed for page table\n", __LINE__, PageTableBufferSize));
    PageTableBuffer = AllocatePageTableMemory (EFI_SIZE_TO_PAGES (PageTableBufferSize));
    ASSERT (PageTableBuffer != NULL);
    Status = PageTableMap (&PageTable, PagingMode, PageTableBuffer, &PageTableBufferSize, StackBase, SIZE_4KB, &MapAttribute, &MapMask);
    ASSERT (Status == RETURN_SUCCESS);
    ASSERT (PageTableBufferSize == 0);
  }
  if (PcdGetBool (PcdSetNxForStack)) {
    ...
    MapMask.Bits.Nx = 1;
    PageTableBufferSize = 0;
    Status = PageTableMap (&PageTable, PagingMode, NULL, &PageTableBufferSize, StackBase, StackSize, &MapAttribute, &MapMask);
    ASSERT (Status == RETURN_BUFFER_TOO_SMALL);
    DEBUG ((DEBUG_INFO, "DxeIpl(%d): 0x%x bytes needed for page table\n", __LINE__, PageTableBufferSize));
    PageTableBuffer = AllocatePageTableMemory (EFI_SIZE_TO_PAGES (PageTableBufferSize));
    ASSERT (PageTableBuffer != NULL);
    Status = PageTableMap (&PageTable, PagingMode, PageTableBuffer, &PageTableBufferSize, StackBase, StackSize, &MapAttribute, &MapMask);
    ASSERT (Status == RETURN_SUCCESS);
    ASSERT (PageTableBufferSize == 0);
  }
