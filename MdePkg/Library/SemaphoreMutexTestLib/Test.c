...
)
{
  ...
  MutexAcquire (ApContext->m, 0);
  MutexAcquire (ApContext->m, 0);
  SerialPortWrite (B,
    AsciiSPrint (B, sizeof (B), "[%x]: I am here!\n", GetApicId ())
  );
  MutexRelease (ApContext->m);

  SerialPortWrite (B,
    AsciiSPrint (B, sizeof (B), "[%x]: I am sleeping...!\n", GetApicId ())
  );
  MicroSecondDelay (5000000);
  SemaphoreRelease (ApContext->s);
  MutexRelease (ApContext->m);

  ASSERT (MutexRelease (ApContext->m) == RETURN_ACCESS_DENIED);
}
...
)
{
  ...
  SemaphoreInitialize (&s, 7);

  for (i = 0; i < 7; i++) {
    RStatus = SemaphoreAcquire (&s, 0);
    ASSERT_RETURN_ERROR (RStatus);
  }
  ASSERT (SemaphoreAcquireOrFail (&s) == FALSE);
  for (i = 0; i < 7; i++) {
    SemaphoreRelease (&s);
  }
