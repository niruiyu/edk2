...
static SMM_PERF_AP_PROCEDURE_PERFORMANCE  *mSmmMpProcedurePerformance = NULL;
  ...
  )
{
  ...
  for (CpuIndex = 0; CpuIndex < NumberofCpus; CpuIndex++) {
    for (MpProcecureId = 0; MpProcecureId < SMM_MP_PERF_PROCEDURE_ID (SmmMpProcedureMax); MpProcecureId++) {
      if (mSmmMpProcedurePerformance[CpuIndex].Start[MpProcecureId] != 0) {
        PERF_START (NULL, gSmmMpPerfProcedureName[MpProcecureId], NULL, mSmmMpProcedurePerformance[CpuIndex].Start[MpProcecureId]);
...
{
  mSmmMpProcedurePerformance[CpuIndex].Start[MpProcedureId] = GetPerformanceCounter ();
}
