  ...
  )
{
  ...
  for (Index = 0; RelaEntrySize * Index < RelaSize; Index++, Rela = ELF_NEXT_ENTRY (Elf32_Rela, Rela, RelaEntrySize)) {
    ...
    //
    Ptr32 = (UINT32 *)(UINTN)(Rela->r_offset + Delta);
  ...
  )
{
  ...
  //
  // 1. Relocate DYN ELF using the relocation section pointed by dynamic section
  //
  ...
  //
  // 2. EXEC ELF: Fix up the delta between actual image address and preferred image address.
  //
  ...
  for ( Index = 0, RelShdr = (Elf32_Shdr *) (ElfCt->FileBase + Ehdr->e_shoff)
    ...
    if (!IsTextShdr(Shdr) && !IsDataShdr(Shdr)) {
      //
      // Only fix up text/data sections.
      //
      continue;
    }
    ProcessRelocation32 (
